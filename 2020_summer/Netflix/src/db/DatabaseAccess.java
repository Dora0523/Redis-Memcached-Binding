...
public class DatabaseAccess extends Thread {
	...
	static int percentRead = 95;
	static int percentInsert = 5;
	...
	void executeInsert() {
		int accid = (int) ((Math.random() * 2)); // Generate random number
		accid=0;
	...
	void getAvailableMedia() {
		...
		try {
			...
			if (jedis.lrange(Integer.toString(querySQL.hashCode()), 0, -1).size() == 0) {
				...
				jedis.expire(Integer.toString(querySQL.hashCode()),2);
